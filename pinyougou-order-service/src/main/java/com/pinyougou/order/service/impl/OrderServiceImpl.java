...
public class OrderServiceImpl implements OrderService {
	...
	public void add(TbOrder order) {
		...
		for(Cart cart:cartList){
			long orderId = idWorker.nextId();  //获取主键ID
			...
			orderIdList.add(orderId+"");  //添加到订单ID列表（支付日志）
			total_money+=money;//累加到总金额（支付日志）
		}
		...
		redisTemplate.boundHashOps("cartList").delete(order.getUserId()); //删除订单缓存
	...
	public PageResult findPage(TbOrder order, int pageNum, int pageSize) {
		...
		public void updateOrderStatus(String out_trade_no, String transaction_id) {
			...
			payLog.setPayTime(new Date()); //交易时间
			payLog.setTradeState("1"); //已支付
			payLog.setTransactionId(transaction_id); //交易流水号
			
			payLogMapper.updateByPrimaryKey(payLog); //修改数据库
			...
			for(String orderId:orderIds ) {
				...
				if (order!=null) {
					...
					orderMapper.updateByPrimaryKey(order); //更新数据库
				}
			}
			//清除支付日志redis缓存
